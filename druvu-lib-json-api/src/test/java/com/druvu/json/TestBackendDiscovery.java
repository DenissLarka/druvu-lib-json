...
public class TestBackendDiscovery {
    ...
    public void failsWithClearMessageWhenNoBackendPresent() {
        IllegalStateException e = Assert.expectThrows(IllegalStateException.class, Json::object);
        Assert.assertTrue(e.getMessage().contains("No JSON backend found"), e.getMessage());
    }

    @Test
    public void parsingNeedsABackendToo() {
        IllegalStateException e = Assert.expectThrows(IllegalStateException.class, () -> Json.parse("{}"));
        Assert.assertTrue(e.getMessage().contains("No JSON backend found"), e.getMessage());
    }

    @Test
    public void failureIsNotCachedAndThrowsAgain() {
        Assert.expectThrows(IllegalStateException.class, Json::array);
        Assert.expectThrows(IllegalStateException.class, Json::array);
    }
}
