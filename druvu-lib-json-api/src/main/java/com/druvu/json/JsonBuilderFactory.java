...
public class JsonBuilderFactory {
	 ...
	 * <p>Implement this interface and register it via a
	 * {@code com.druvu.lib.loader.ComponentFactory} service to add a new backend.
	 * The {@code of} methods never receive {@code null}; null values are turned into
	 * {@link #nullNode()} by the builders.
	 * <p>Backends without native date/time or character node types can rely on the
	 * default methods, which convert those values to ISO-formatted strings. Backends
	 * with richer node types (e.g. native timestamps) may override them.
	 *
	 * @param <N> the backend's native node type
	...
	/**
	 * Lazily discovers the backend, so that explicit-backend usage works without any
	 * backend registered. A failed discovery is not cached: it throws again on the
	 * next call, with the original cause attached.
	 */
					...
					try {
						...
						throw new IllegalStateException(
								"No JSON backend found. Add a backend module (e.g. com.druvu:fluent-json-gson) "
										+ "to the classpath, or pass a Backend to the factory methods explicitly.", e);
					}
	...
	private static class Impl<N, P> implements JsonObjectBuilder<P, N>, JsonArrayBuilder<P, N> {
		...
		/**
		 * Extracts the native node from another builder, refusing nodes that belong
		 * to a different backend — they would corrupt this builder's tree.
		 */
		private N graft(JsonBuilder builder) {
			if (!(builder instanceof Impl)) {
				throw new IllegalArgumentException(
						"Unsupported JsonBuilder implementation: " + builder.getClass().getName());
			}
